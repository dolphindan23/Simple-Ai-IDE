...
echo "=========================================="
echo "  SimpleAide Docker Stop (Ollama)"
echo "=========================================="
echo ""

# Check for running containers
RUNNING=$(docker compose -f docker-compose.gpu.yml --profile ollama ps -q 2>/dev/null | wc -l)

if [ "$RUNNING" -eq 0 ]; then
    echo "[*] No Ollama profile containers are running."
    exit 0
fi
...
if [ "$CLEAN_VOLUMES" = true ]; then
    ...
    docker compose -f docker-compose.gpu.yml --profile ollama down -v
    echo ""
    ...
    echo "    ./ollama_docker_stop_all.sh --clean"
fi
